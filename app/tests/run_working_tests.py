...
if __name__ == '__main__':
    ...
    for test_name in [
        ...
        test_suite.addTest(unittest.TestLoader().loadTestsFromName(test_name, TestBasicFunctionality))
    ...
    test_suite.addTest(unittest.TestLoader().loadTestsFromTestCase(TestVisualizationFunctions))

    # Adicionar o teste de interface de treinamento que funciona
    from test_streamlit_interface import TestStreamlitInterface
    test_suite.addTest(unittest.TestLoader().loadTestsFromName('test_training_interface', TestStreamlitInterface))
