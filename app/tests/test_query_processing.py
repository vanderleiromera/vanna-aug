...
class TestQueryProcessing(unittest.TestCase):
    ...
    def test_normalize_question_with_numbers(self):
        ...
        # Verificar se a função normalizou corretamente a pergunta
        self.assertEqual(normalized, "Mostre os X principais clientes com vendas acima de Y reais")
    ...
    def test_adapt_sql_to_values(self):
        ...
        expected_sql = "SELECT * FROM sales WHERE date >= NOW() - INTERVAL '60 days' LIMIT 20"
    ...
    def test_find_similar_questions(self, mock_get_similar):
        ...
        # Chamar a função
        result = self.vanna.get_similar_question_sql("Mostre as vendas dos últimos 60 dias")
    ...
    def test_process_question_with_similar(self, mock_normalize, mock_adapt, mock_get_similar):
        ...
        mock_adapt.return_value = "SELECT * FROM sales WHERE date >= NOW() - INTERVAL '60 days'"
        ...
        if similar_questions and "Question:" in similar_questions[0] and "SQL:" in similar_questions[0]:
        ...
        self.assertEqual(sql, "SELECT * FROM sales WHERE date >= NOW() - INTERVAL '60 days'")
