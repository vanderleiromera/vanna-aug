...
def main():
    ...
    print(f"OPENAI_EMBEDDING_MODEL: {os.getenv('OPENAI_EMBEDDING_MODEL', 'Não definido')}")
    ...
    if hasattr(vn, 'client') and vn.client:
        ...
        if hasattr(vn, 'embedding_function') and vn.embedding_function:
            print(f"Função de embedding definida: {type(vn.embedding_function).__name__}")
            
            # Verificar se o modelo está definido na função de embedding
            if hasattr(vn.embedding_function, 'model_name'):
                print(f"Modelo na função de embedding: {vn.embedding_function.model_name}")
        else:
