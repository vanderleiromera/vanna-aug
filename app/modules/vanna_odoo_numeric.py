...
class VannaOdooNumeric(VannaOdoo):
    ...
    def __init__(self, config=None):
        ...
        self.numeric_patterns = [
            ...
            (r'(os|as|mostrar|mostre|exibir|exiba)\s+(\d+)\s+(primeiros|melhores|principais)', 'top_n'),
                ...
                if value_type == 'top_n':
                    ...
                    try:
                        ...
                        if len(match.groups()) >= 3:
                            ...
                            print(f"[DEBUG] Extraído número {match.group(2)} do padrão 'mostre os X principais'")
                        elif len(match.groups()) >= 2:
                            # Tentar converter o grupo 2 para inteiro (padrão "top 10")
                            try:
                                values[value_type] = int(match.group(2))
                                print(f"[DEBUG] Extraído número {match.group(2)} do padrão 'top X'")
                            except ValueError:
                                # Se falhar, tentar o grupo 1 (padrão "10 principais")
                                values[value_type] = int(match.group(1))
                                print(f"[DEBUG] Extraído número {match.group(1)} do padrão 'X principais'")
                    except (ValueError, IndexError) as e:
                        # Se ambos falharem, usar um valor padrão
                        values[value_type] = 10
                        print(f"[DEBUG] Usando valor padrão 10 para padrão top_n. Erro: {e}")
                else:
                    # Para outros padrões, o número está no grupo 1
                    values[value_type] = float(match.group(1)) if '.' in match.group(1) else int(match.group(1))
    ...
    def normalize_question(self, question: str) -> Tuple[str, Dict[str, Any]]:
        ...
        for value_type, value in values.items():
            if value_type == 'days':
                normalized_question = re.sub(r'\d+\s+(dia|dias|day|days)', '30 dias', normalized_question, flags=re.IGNORECASE)
            elif value_type == 'months':
                normalized_question = re.sub(r'\d+\s+(mês|meses|mes|meses|month|months)', '3 meses', normalized_question, flags=re.IGNORECASE)
            elif value_type == 'years':
                normalized_question = re.sub(r'\d+\s+(ano|anos|year|years)', '1 ano', normalized_question, flags=re.IGNORECASE)
            elif value_type == 'year':
                normalized_question = re.sub(r'(?:de|em|do|no|para|por)\s+\d{4}', 'em 2024', normalized_question, flags=re.IGNORECASE)
                normalized_question = re.sub(r'(?:valor|vendas|vendidos)\s+(?:de|em|do|no|para|por)\s+\d{4}', 'vendidos em valor de 2024', normalized_question, flags=re.IGNORECASE)
                normalized_question = re.sub(r'\d{4}(?:\s+|$)', '2024', normalized_question, flags=re.IGNORECASE)
            elif value_type == 'quantity':
                normalized_question = re.sub(r'\d+\s+produtos', '50 produtos', normalized_question, flags=re.IGNORECASE)
                normalized_question = re.sub(r'nível\s+de\s+estoque\s+de\s+\d+', 'nível de estoque de 50', normalized_question, flags=re.IGNORECASE)
            elif value_type == 'top_n' or value_type == 'limit':
                normalized_question = re.sub(r'(top|primeiros|melhores)\s+\d+', 'top 10', normalized_question, flags=re.IGNORECASE)
                normalized_question = re.sub(r'\d+\s+(primeiros|melhores|principais)', '10 primeiros', normalized_question, flags=re.IGNORECASE)
                ...
                normalized_question = re.sub(pattern, 'os 10 principais', normalized_question, flags=re.IGNORECASE)
    ...
    def ask(self, question: str) -> str:
        ...
        if "nivel de estoque" in question.lower() and "produtos" in question.lower() and "vendidos em valor" in question.lower():
            print("[DEBUG] Detectada pergunta sobre nível de estoque de produtos vendidos em valor")
        ...
        if "vendas" in question.lower() and "mês" in question.lower() and "year" in values:
