...
def evaluate_sql_quality(sql: str) -> Dict[str, Any]:
    ...
    best_practices_score, best_practices_issues, warnings, suggestions = check_best_practices(sql)
    ...
    performance_score, performance_issues, performance_warnings, performance_suggestions = check_performance(sql)
...
def check_basic_syntax(sql: str) -> Tuple[int, List[str]]:
    ...
    if not re.match(r'^\s*(SELECT|INSERT|UPDATE|DELETE|WITH)', sql.strip().upper()):
        score -= 15
        issues.append("A consulta não começa com uma palavra-chave SQL válida (SELECT, INSERT, UPDATE, DELETE, WITH)")
...
def check_best_practices(sql: str) -> Tuple[int, List[str], List[str], List[str]]:
    ...
    if re.search(r'FROM\s+\w+\s+\w+', sql.upper()) or re.search(r'JOIN\s+\w+\s+\w+', sql.upper()):
        ...
        if 'JOIN' in sql.upper():
            score -= 5
            suggestions.append("Use aliases para tabelas em consultas com JOIN para melhorar a legibilidade")
    
    # Verificar se há GROUP BY sem agregação
    if 'GROUP BY' in sql.upper() and not any(x in sql.upper() for x in ['COUNT(', 'SUM(', 'AVG(', 'MAX(', 'MIN(']):
        score -= 5
    ...
    if 'ORDER BY' in sql.upper() and 'LIMIT' not in sql.upper():
        suggestions.append("Considere adicionar LIMIT à consulta com ORDER BY para limitar o número de resultados")
...
def check_performance(sql: str) -> Tuple[int, List[str], List[str], List[str]]:
    ...
    if subquery_count > 2:
        score -= 5
        warnings.append(f"A consulta contém {subquery_count} subconsultas, o que pode afetar o desempenho")
    ...
    if re.search(r'WHERE\s+\w+\s*\(\s*\w+\s*\)', sql.upper()):
        score -= 5
        warnings.append("A consulta usa funções em cláusulas WHERE, o que pode impedir o uso de índices")
    ...
    if 'DISTINCT' in sql.upper() and 'GROUP BY' in sql.upper():
        score -= 5
        warnings.append("A consulta usa DISTINCT com GROUP BY, o que pode ser redundante")
    ...
    if join_count > 5:
        score -= 5
        warnings.append(f"A consulta contém {join_count} JOINs, o que pode afetar o desempenho")
        suggestions.append("Considere simplificar a consulta ou dividir em consultas menores")
...
def check_security(sql: str) -> Tuple[int, List[str], List[str]]:
    ...
    if '--' in sql or '/*' in sql:
        score -= 10
        warnings.append("A consulta contém comentários, o que pode indicar tentativa de injeção SQL")
    
    # Verificar se há comandos perigosos
    dangerous_commands = ['DROP', 'TRUNCATE', 'DELETE FROM', 'UPDATE', 'ALTER', 'GRANT', 'REVOKE']
    ...
    if 'UNION' in sql.upper():
        score -= 5
        warnings.append("A consulta contém UNION, verifique se não é uma tentativa de injeção SQL")
